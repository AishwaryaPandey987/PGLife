window.addEventListener("load", function () {
 ...
 ///////////var is_booked= document.getElementsByClassName("btn")[0];
  /////////is_booked.addEventListener("click", function (event) {
 ///////     var XHR = new XMLHttpRequest();

      // On success
    //////  XHR.addEventListener("load", toggle_booked_success);

      // On error
    /////////  XHR.addEventListener("error", on_error);

      // Set up request
    ///////  XHR.open("GET", "api/toggle_booked.php?property_id=" + property_id);
 ...
 // var toggle_booked_success = function (event)  {
   //// document.getElementById("loading").style.display = 'none';
  
    ///var response = JSON.parse(event.target.responseText);
    ///////if (response.success) {
        /////var is_booked = document.getElementsByClassName("btn");
        //////var booked_user_count = document.getElementsByClassName("booked-user-count")[0];
  
     ////   if (response.is_booked) {
       //     is_booked.classList.add("btn-primary");
        //    is_booked.classList.remove("btn-non-primary");
          //////  booked_user_count.innerHTML = parseFloat(interested_user_count.innerHTML) + 1;
      /////  } else {
         //   is_booked.classList.add("btn-non-primary");
           // is_booked.classList.remove("btn-primary");
           // booked_user_count.innerHTML = parseFloat(interested_user_count.innerHTML) - 1;
